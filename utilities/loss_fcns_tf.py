...
from diff_tf import ddx, ddy, ddz
...
class Loss:
    def __init__(self,nx,ny,nz,Lx,Ly,Lz,inc_mom = False):
        ...
        for i, key in enumerate(self.keys):
            self.ground_truth[key] = np.empty((nz,1), dtype = np.float32, order = 'F')
            self.current_state[key] = np.empty((nz,1), dtype = np.float32, order = 'F')

        # initialize derivative operator
        self.dop = DiffOps(nx = nx, ny = ny, nz = nz, Lx = Lx, Ly = Ly, Lz = Lz)
        
        # Allocate memory for the flow variables
        self.u     = np.empty((nx,ny,nz),   dtype = np.float32, order = 'F')
        self.v     = np.empty((nx,ny,nz),   dtype = np.float32, order = 'F')
        self.w     = np.empty((nx,ny,nz),   dtype = np.float32, order = 'F')
        self.A     = np.empty((nx,ny,nz,3), dtype = np.float32, order = 'F')
        if self.inc_mom: 
            self.p     = np.empty((nx,ny,nz),   dtype = np.float32, order = 'F')
            self.tauij = np.empty((nx,ny,nz,6), dtype = np.float32, order = 'F')
            self.B     = np.empty((nx,ny,nz),   dtype = np.float32, order = 'F')

    def mean_square(self,f):
        return tf.multiply(f,f)
    ...
    def MSE(self,f,g):
        ...
        #            i,g.shape[i])
        L = self.mean_square(tf.subtract(f,g))
        return L 
    ...
    def xy_avg(self,f):
        ...
        return tf.math.reduce_mean(f, axis = (0,1), keepdims = False, name = 'xy_avg')
    
    def fluct(self,u):
        nx, ny, nz = tuple(u.shape().as_list())
        self.confirm_dimensions(nx, ny, nz)
        uAvg = self.xy_avg(u)
...
#        return None
    ...
    def extract_3Dfields_from_output_layer(self, Yhat_in, Yin, example):
        nx, ny, nz = self.nx, self.ny, self.nz
        if self.inc_mom:
            Yhat = tf.reshape(Yhat, [-1, nx, ny, nz, 10])
            nprofs = 14
        else:
    ...
    def L_mass(self):
        return self.mean_square(ddx(self.u) + ddy(self.v) + ddz(self.w))
...
def test_extract_field_variables_from_input_layer(nx,ny,nz):
    ...
    X = np.empty((nx,ny,nz,4), dtype = np.float32)
...
def test_extract_3Dfields_from_output_layer(nx,ny,nz):
    ...
    Yhat     = np.empty((nx,ny,nz,10), dtype = np.float32)
    Yhat[:,:,:,:6] = tauij
    Yhat[:,:,:,6:9] = A
    Yhat[:,:,:,9] = B
...
def load_data_for_loss_tests(datadir,nzC,nzF,tidx,tidy,navg, inc_prss = True):
    ...
    Yhat = np.ones((nx*ny*nz*3,1), dtype = np.float32, order = 'F')
    return X, Y, Yhat
